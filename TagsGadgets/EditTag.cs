...
namespace TagGadgets
{
    ...
    class EditTag : IExternalCommand
    {
        public Result Execute(ExternalCommandData revit, ref string message, ElementSet elements)
        {
            ...
            try
            {
                selRefs = docUI.Selection.PickObjects(ObjectType.Element, new IndependentTagFilter(), "Выбери элемент с коннекторами");
            }
            catch (Exception ex)
            {
                if(ex is OperationCanceledException) return Result.Cancelled;
            }
            ...
            using (Transaction tr = new Transaction(docUI.Document, "pre-modify tags"))
            {
                ...
                {
                    if (!(docUI.Document.GetElement(selRef.ElementId) is IndependentTag elem)) continue;
                    ...
                    {
                        var displacementGroup = doc.GetElement(displacementGroupId) as DisplacementElement;
                        var groupOffset       = GetSummaryOffset(displacementGroup);
                        elem.TagHeadPosition  = TagHelpers.GetPointOnHostElement(doc, elem) - groupOffset;
                    }
                    else
        ...
        private XYZ GetSummaryOffset(DisplacementElement displacementElement)
        {
            ...
            if (displacementElement.ParentId != ElementId.InvalidElementId)
                xYZ += GetSummaryOffset(displacementElement.Document.GetElement(displacementElement.ParentId) as DisplacementElement);
            return xYZ;
        }
        public class IndependentTagFilter : ISelectionFilter
        {
            ...
            {
                if (elem is IndependentTag) return true;
                return false;
            }
