...
namespace TagGadgets
{
    ...
    class CreateTags : IExternalCommand
    {
        public Result Execute(ExternalCommandData revit, ref string message, ElementSet elements)
        {
            ...
            if (selIDs == null || selIDs.Count == 0) return Result.Cancelled;

            var point     = uiDoc.Selection.PickPoint();
            var tags      = new List<IndependentTag>();
            var offset    = UnitUtils.ConvertToInternalUnits(0, DisplayUnitType.DUT_MILLIMETERS);
            ...
            using (Transaction tr = new Transaction(doc))
            {
                ...
                foreach (var selID in selIDs)
                {
                    ...
                    var endPoint = TagHelpers.GetPointOnHostElement(doc, doc.GetElement(refEl.ElementId));
        ...
        private IndependentTag CreateIndependentTag(Document document, Reference reference, XYZ pastePoint, XYZ endPoint)
        {
            var view           = document.ActiveView;
            var tagMode        = TagMode.TM_ADDBY_CATEGORY;
            var tagOrientation = TagOrientation.Horizontal;

            IndependentTag newTag = IndependentTag.Create(document, 
                                                            view.Id, 
                                                            reference, 
                                                            true, 
                                                            tagMode, 
