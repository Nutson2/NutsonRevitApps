...
namespace TagGadgets
{
    internal class TagLeader
    {
        ...
        public TagLeader(IndependentTag tag, Document doc)
        {
            ...
            _side            = ((_currentView.CropBox.Max+ _currentView.CropBox.Min)/ 2.0).X <= _leaderEnd.X ? 
                                LegSide.Right : 
                                LegSide.Left;
            GetTagDimension();
        }
        ...
        private void UpdateLeaderPosition()
        {
            ...
            _elbowPosition = _tagCenter + new XYZ(xyz.X - xyz.Y * Math.Tan(num2 * Math.PI / 4.0), 0.0, 0.0);

            _endLine       = _leaderEnd.DistanceTo(_elbowPosition) <= _doc.Application.ShortCurveTolerance ?
            ...
            if (_elbowPosition.DistanceTo(_tagCenter) > _doc.Application.ShortCurveTolerance)
                ...
                _baseLine  = Line.CreateBound(new XYZ(0.0, 0.0, 0.0), 
                                            new XYZ(0.0, 0.0, 1.0));
        }
        ...
        private void GetTagDimension()
        {
            ...
            _tagHeight = cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Max).Y- 
                        cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Min).Y;
            
            _tagWidth  = cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Max).X - 
                        cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Min).X;

            _tagCenter = (cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Max)+ 
                           cropBox.Transform.Inverse.OfPoint(boundingBoxXyz.Min))/ 2.0;
        ...
        public static Element GetTaggedElement(Document doc, IndependentTag tag) =>
                tag.TaggedElementId.HostElementId != ElementId.InvalidElementId ?
                    ...
                    (doc.GetElement(tag.TaggedElementId.LinkInstanceId) as RevitLinkInstance).GetLinkDocument().GetElement(tag.TaggedElementId.LinkedElementId);
