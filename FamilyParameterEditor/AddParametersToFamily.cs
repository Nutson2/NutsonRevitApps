...
namespace FamilyParameterEditor.EditFamiliesParameters.ViewModel
{
    ...
    public class AddParametersToFamily : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (!famDoc.IsFamilyDocument) return Result.Cancelled;

            var FM = famDoc.FamilyManager;

            var famParamName                = new string[] { "LT_SPE", "id_SystemName", "id_category", "SystemName", "category" };
            var famParamType                = new ParameterType[] { ParameterType.Text, ParameterType.Integer, ParameterType.Integer, ParameterType.Text, ParameterType.Text };
            List<FamilyParameter> newparams = new List<FamilyParameter>();

            var sharedParamNames = new string[] { "SPE_Code_Category", "SPE_Code_Classification", "SPE_Code_Description", "SPE_Code_SystemType" };
            ...
            var dictFormulas=new Dictionary<string, string>() 
            {
                ...
                {"SystemName","size_lookup(LT_SPE, \"SystemName\", \"Нет в каталоге\", id_SystemName, id_category)"},
                {"category","size_lookup(LT_SPE, \"category\", \"Нет в каталоге\", id_SystemName, id_category)" },

                {"SPE_Code_Category","size_lookup(LT_SPE, \"SPE_Code_Category\", \"Нет в каталоге\", id_SystemName, id_category)" },
                {"SPE_Code_Classification","size_lookup(LT_SPE, \"SPE_Code_Classification\", \"Нет в каталоге\", id_SystemName, id_category)" },
                {"SPE_Code_Description","size_lookup(LT_SPE, \"SPE_Code_Description\", \"Нет в каталоге\", id_SystemName, id_category)" },
                {"SPE_Code_SystemType","size_lookup(LT_SPE, \"SPE_Code_SystemType\", \"Нет в каталоге\", id_SystemName, id_category)" },
            };
            ...
            using (var tr = new Transaction(famDoc, "addFormulas"))
            {
                ...
                foreach (var fparam in newparams)
                    {
                        if (!dictFormulas.ContainsKey(fparam.Definition.Name)) continue;
        ...
        private static void CreateSizeTable(Document famDoc)
        {
            ...
            if (fod.ShowDialog() == System.Windows.Forms.DialogResult.Cancel) return;
            ...
            try
            {
                using (var tr = new Transaction(famDoc, "addLT"))
                {
                    ...
                    var famSizeTablemanager = FamilySizeTableManager.GetFamilySizeTableManager(famDoc, famDoc.OwnerFamily.Id);
        ...
        private ExternalDefinition[] GetExternalDefinitions(Document document, string[] paramNames) 
        {
            ...
            return gr.Definitions.Where(x => paramNames.Contains(x.Name)).Cast<ExternalDefinition>().ToArray();
        }
    }
}
