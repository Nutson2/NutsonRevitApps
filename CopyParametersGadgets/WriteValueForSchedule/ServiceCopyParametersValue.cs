...
namespace CopyParametersGadgets.Command
{
    public class ServiceCopyParametersValue
    {
        ...
        public bool CopyParamValue()
        {
            ...
            using (Transaction tr = new Transaction(doc, "перенос"))
            {
                ...
                foreach (Element el in elements)
                {
                    ...
                    {
                        errors.AppendLine(
                            $"Element ID: {element.Id} || Message: {ex.Message} || StackTrace: {ex.StackTrace}"
                        );
                    }
                }
                tr.Commit();
        ...
        private void SaveErrorsLog()
        {
            ...
            using (
                var stream = new StreamWriter(
                    Path.Combine(path, "Schedule error.txt"),
                    false,
                    Encoding.Default
                )
            )
        ...
        private void SetSystemName(Element el)
        {
            ...
            var elType = doc.GetElement(
                ownerFam.get_Parameter(BuiltInParameter.RBS_PIPING_SYSTEM_TYPE_PARAM).AsElementId()
            );
            if (elType != null)
                el.CopyValueBetweenElements(
                    elType,
                    BuiltInParameter.ALL_MODEL_DESCRIPTION,
                    "ИОС_Наименование системы"
                );
        }
        ...
        private void SetInsulationSizeMark(CacheData cachedData, PipeInsulation el)
        {
            ...
            var insTypeComment = insulationType
                .get_Parameter(BuiltInParameter.ALL_MODEL_TYPE_COMMENTS)
                .AsString();
            ...
            if (insTypeComment.StartsWith("Сортамент"))
            {
                ...
                {
                    currentTypeSizes = insTypeComment
                        .Split(':')
                        .Skip(1)
        ...
        private void SetFamilyInstanceSizeMark(FamilyInstance el)
        {
            ...
            if (pipeSettings.SizePrefix != "")
                SizeString.Replace(pipeSettings.SizePrefix, "");
            if (pipeSettings.SizeSuffix != "")
                SizeString.Replace(pipeSettings.SizeSuffix, "");

            var newSize_msv = SizeString
                .Split(
                    pipeSettings.ConnectorSeparator.ToArray(),
        ...
        private void SetGroupParamValue(CacheData cachedData, Element el)
        {
            ...
            if (!(el is FamilyInstance famInstance))
                return;
        ...
        private void SubSetGroupParamValue(CacheData cachedData, Element el)
        {
            ...
            )
            {
                ...
                if (paramOName == null)
                    paramOName = elType.LookupParameter("О_Наименование");
                if (paramIOSSize == null)
                    paramIOSSize = elType.LookupParameter("ИОС_Размер_Специф");
                if (paramOObosnachenie == null)
                    paramOObosnachenie = elType.LookupParameter("О_Обозначение");
                if (paramOZavod == null)
                    paramOZavod = elType.LookupParameter("О_Завод-изготовитель");
            }

            string CategoryNameGost = string.Empty;
            if (
                cachedData.GostCategoryNames.ContainsKey(
                    (BuiltInCategory)el.Category.Id.IntegerValue
                )
            )
            {
                CategoryNameGost = cachedData.GostCategoryNames[
                    (BuiltInCategory)el.Category.Id.IntegerValue
                ];
            ...
            paramIOSNamefull.TrySetValue(
                paramOName.TryAsString() + " Ø" + paramIOSSize.TryAsString()
            );
            ...
            if (ElCategory == null || ElStage == null || paramSorting == null)
                return;

            var stringForSorting = string.Format(
            ...
            var stringForSortingBOP = string.Format(
                "{0}_{1}_{2}",
                stringForSorting,
        ...
        {
            public readonly Dictionary<ElementId, Dictionary<int, int[]>> InsulationSizes =
                new Dictionary<ElementId, Dictionary<int, int[]>>();
