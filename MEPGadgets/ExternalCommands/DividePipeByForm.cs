...
namespace MEPGadgets
{
    ...
    class DividePipeByForm : IExternalCommand
    {
        public Result Execute(ExternalCommandData revit, ref string message, ElementSet elements)
        {
            ...
            IList<Element> pipes = collector.OfClass(typeof(Pipe)).WhereElementIsNotElementType().ToElements();
            ...
            using (Transaction tr = new Transaction(doc, "режем"))
            {
                ...
                foreach (Element el in pipes)
                {
                    ...
                    if (curpipe.GroupId.IntegerValue != -1) continue;

                    var LCurve           = curpipe.Location as LocationCurve;
                    var pipeLine         = LCurve.Curve as Line;
                    var origin           = pipeLine.GetEndPoint(0);

                    var intersections = refInetrsector.Find(origin, pipeLine.Direction)
                        .Where(x => x.Proximity <= pipeLine.Length - UnitUtils.ConvertToInternalUnits(10, DisplayUnitType.DUT_MILLIMETERS))
                        .ToList();
                    ...
                    {
                        if (dictBreakPoint.ContainsKey(intersect.Proximity)
                            || Math.Round(intersect.Proximity, 6) == 0) continue;

                        XYZ breakpoint = origin.Add(vector.Multiply(intersect.Proximity));

                        foreach (Pipe nPipe in CreatedPipe)
                        {
                            IntersectionResult res = ((Line)((LocationCurve)nPipe.Location).Curve).Project(breakpoint);
                            if (Math.Round(res.Distance, 6) != 0) continue;
                            curpipe = nPipe;
                            break;
                        }
