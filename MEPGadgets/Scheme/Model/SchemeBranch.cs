...
{
    public partial class SchemeBranch : ObservableObject
    {
        [ObservableProperty]
        ...
        [ObservableProperty]
        private bool _analyzingFinished;
        ...
        {
            get { return Elements?.Cast<object>().Concat(NextBranches); }
        }
        ...
        {
            if (
                !element.LookupParameter("Имя системы").AsString().Contains(systemScheme.SystemName)
            )
                return;
        ...
        private IList<Element> GetNextElements(Element element)
        {
            ...
            foreach (Connector c in CM.Connectors)
            {
                if (!c.IsConnected)
                    continue;
                if (c.Domain != Domain.DomainPiping || c.MEPSystem == null)
                    continue;
                if (!c.MEPSystem.Name.Contains(systemScheme.SystemName))
                    continue;
                if (
                    systemScheme.ConnectorsInScheme.ContainsKey(
                        c.Owner.Id.ToString() + c.Id.ToString()
                    )
                )
                    continue;
        ...
        {
            if (e.PropertyName != nameof(AnalyzingFinished))
                return;
            if (NextBranches.Any(br => br.AnalyzingFinished == false))
                return;
        ...
        private Element GetConnectedElement(Connector curConnector)
        {
            ...
            if (systemScheme.ConnectorsInScheme.ContainsKey(key))
                return null;
            systemScheme.ConnectorsInScheme.Add(key, curConnector);

            var respondedConnector = GetRespondingConnector(curConnector);

            if (respondedConnector == null)
                return null;
            var respondConnectorKey =
                respondedConnector.Owner.Id.ToString() + respondedConnector.Id.ToString();
        ...
        {
            var respondCon = con
                .AllRefs.Cast<Connector>()
                .Where(x => x.Owner.Id != con.Owner.Id)
                .Where(x =>
                    x.Owner.Category.Id.IntegerValue != (int)BuiltInCategory.OST_PipeInsulations
                )
                .Where(x =>
                    x.Owner.Category.Id.IntegerValue != (int)BuiltInCategory.OST_PipingSystem
                )
                .ToList();
